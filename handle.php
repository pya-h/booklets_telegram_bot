                ...
                if (!$user[DB_USER_ACTION]) {
                    ...
                    switch ($user[DB_USER_ACTION]) {
                        ...
                        case ACTION_SET_SAMPLE_TITLE:
                            ...
                            if (isset($params[1])) {
                                $response = backupSample($params[1], $data);
                            }
                            // returns error
                            else {
                                $response = 'مشکلی حین ارسال نمونه سوال به کانال بک آپ پیش آمد!';
                            }
...
function handleCallbackQuery(&$update)
{
    ...
    if ($raw_data == -1) {
        ...
        $action = $params[1] ?? null; // TODO: => must be action

        switch ($action) {
            case 'sh':
                // user wants to see admin message
                $answer = validateInlineData($params, 'msg', 'admin', 'rpm');
                if (!$answer) {
                    callMethod(
                        METH_COPY_MESSAGE,
            ...
            case "gb":
            case "gs":
                $answer = validateInlineData($params, "sel");
                if ($answer) {
                    break;
                }
