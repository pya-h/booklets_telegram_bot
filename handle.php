    ...
    if ($target_id) {
        ...
        if (updateUserMode($target_id, $mode, $teacher_id, $mode == TEACHER_USER ? $teacher_name : null)) {
            ...
            callMethod(
                ...
                $target_id,
                TEXT_TAG,
                $mode != TA_USER ? "تبریک! اکانت شما به دسترسی $position_title ارتقا پیدا کرد." : "تبریک اکانت شما به عنوان حل تمرین استاد $teacher_name ثبت شد.",
                KEYBOARD,
                getMainMenu($mode)
            );
...
function handleUpdates(&$update)
{
    ...
    $channels = array(
        FIRST_2_JOIN_CHANNEL_ID => array('name' => "Persian College", INLINE_URL_TAG => FIRST_2_JOIN_CHANNEL_URL),
    ...
    $channel_list_menu[] = array(array(TEXT_TAG => 'بررسی عضویت', CALLBACK_DATA => jsonifyCallbackData(IA_CHECK_MEMBERSHIP)));
    if ($all_joined) {
        ...
        callMethod(
            ...
            $user_id,
            TEXT_TAG,
            'قبل از هر چیزی لازمه که در کانال های ما جوین شی',
            KEYBOARD,
        ...
        callMethod(
            ...
            $user_id,
            TEXT_TAG,
            'بعد از اینکه عضو کانال های زیر شدی، بررسی عضویت رو بزن:',
            KEYBOARD,
            array(INLINE_KEYBOARD => $channel_list_menu)
        );
    }
}

function validateCategoricalCallbackData(array $data, $required_num_of_keys,...$keys): ?string
{
    $count = 0;
    foreach ($keys as $key) {
        if (isset($data[$key]))
